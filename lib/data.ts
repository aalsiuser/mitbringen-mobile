import type { Store, Product, BasketPlan, BasketStop } from './types'
...
export function optimiseBasket(items: Product[]): BasketPlan {
  if (!items.length) return { stops: [], leftover: [], totalSaving: 0, totalSpend: 0, covered: 0 }

  const byStore: Record<string, Product[]> = {}
  items.forEach((it) => { (byStore[it.store] = byStore[it.store] || []).push(it) })

  const ranked: BasketStop[] = Object.keys(byStore)
    .map((sid) => {
      const list = byStore[sid]
      return { store: STORES[sid], items: list, saving: list.reduce((s, p) => s + savingsOf(p), 0), spend: list.reduce((s, p) => s + p.promo, 0) }
    })
  ...
  if (leftover.length && stops[1]) {
    stops[1] = { ...stops[1], items: [...stops[1].items, ...leftover], saving: stops[1].saving + leftover.reduce((s, p) => s + savingsOf(p), 0), spend: stops[1].spend + leftover.reduce((s, p) => s + p.promo, 0) }
  }
  return { stops, leftover: stops[1] ? [] : leftover, totalSaving: items.reduce((s, p) => s + savingsOf(p), 0), totalSpend: items.reduce((s, p) => s + p.promo, 0), covered: items.length }
}
